...
test('install a version not cached locally via user credentials', async () => {
  ...
  expect(fs.existsSync(`${cacheDir}/foundryvtt-${latestVersion}.zip`)).toBeTruthy();
  expect(fs.existsSync(`${installDir}/foundryvtt`)).toBeTruthy();
  expect(fs.existsSync(`${installDir}/foundrydata`)).toBeTruthy();
  const installedVersion = require(`${installDir}/foundryvtt/resources/app/package.json`).version;
  expect(installedVersion).toBe(latestVersion);
  const { license, signature } = require(`${installDir}/foundrydata/Config/license.json`);
