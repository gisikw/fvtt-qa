...
let PORT = (process.argv.find(arg => arg.match(/^--port=/))||'').split('=')[1] || 30000;
let DATA_PATH = (process.argv.find(arg => arg.match(/^--dataPath=/))||'').split('=')[1];
...
handlers.push([/license/, async (req, res) => {
  let stream = '';
  req.on('data', chunk => { stream += chunk });
  req.on('end', () => { 
...
async function writeLicense(license, cb) {
  ...
  fs.writeFile(LICENSE_FILE, JSON.stringify({
    license
  }, null, 2), cb);
}
